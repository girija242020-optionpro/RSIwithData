...
async function ensureToken() {
  ...
  if (!r.ok || !data.accessToken) throw new Error(`Dhan token generation failed: ${JSON.stringify(data).slice(0,500)}`);
...
async function boot(){
  try{await ensureToken();await loadExpiry();connectMarket();connectDepth([]);setTimeout(refreshChain,500);setInterval(refreshChain,CHAIN_REFRESH_MS);}catch(e){console.error('BOOT:',e.message);setTimeout(boot,5000);}
}
